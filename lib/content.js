...
function createAskPassHelper(token, repoUrl) {
  ...
  const askPassHelper = path.join(__dirname, `.git-askpass${isWin ? '.cmd' : '.sh'}`);
 ...
 */
async function cloneRepo(url, tmpDir, branch, timeout, env) {
  ...
  await execFileAsync(
    'git',
    ['clone', '--depth', '1', '--branch', branch, url, tmpDir],
    {
      timeout,
      stdio: 'pipe',
      env: env || process.env,
...
async function parsePosts(postsDir) {
  ...
  if (mdFiles.length === 0) {
    const err = new Error(`No markdown files found in ${postsDir}`);
    err.code = 'ENOENT';
    throw err;
  }
